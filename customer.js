...
function addToCartEngine(key, launchDirectCheckout) {
    ...
    const cartItem = {
        productId: key, // Track exact Product ID references
        name: finalName,
        price: finalPrice,
        qty: qty,
        img: finalImg
...
function checkoutCart() {
    ...
    cart.forEach(item => {
        ...
        itemsStructure.push({
            productId: item.productId, // Permanent Product ID mapping
            name: item.name,           // Permanent Variant/Product Name mapping
            qty: item.qty,
            price: item.price
        });
    });
    ...
    // ACCURATE DATABASE SYNC TRANSACTION
    const orderPayload = {
    ...
    fetch(FIREBASE_BASE_URL + "orders.json", {
        ...
        let cleanCustomerPhone = phone.replace(/\D/g, '');
        if(cleanCustomerPhone.startsWith('0')) {
            cleanCustomerPhone = '94' + cleanCustomerPhone.substring(1);
        }

        setTimeout(() => {
            const customerWhatsAppInvoiceUrl = `https://wa.me/${cleanCustomerPhone}?text=${encodeURIComponent(orderMsg)}`;
