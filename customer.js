...
function renderProductGrids(data) {
    ...
    Object.keys(data).forEach(key => {
        ...
        // --- PHOTO UPLOAD ERROR FIX: IMAGE ENGINE LAYER ---
        let imgList = [];
        if (product.variants && product.variants.length > 0) {
            product.variants.forEach(v => {
                if (v.image && v.image.trim() !== "") {
                    imgList.push(v.image);
                }
            });
        }
        ...
        // If image links are totally broken or empty
        if (imgList.length === 0) {
            imgList = ['https://via.placeholder.com/300'];
        }
        ...
        const cardContent = `
            <div class="product-card" ${cardStyle}>
                <div class="card-img-container" onclick="openProductView('${key}')" style="cursor:pointer; text-align:center; height:180px; display:flex; align-items:center; justify-content:center; overflow:hidden; background:#f8fafc; border-radius:8px;">
                    <img id="img-${key}" src="${imgList[0]}" alt="Product Image" onerror="this.src='https://via.placeholder.com/300';" style="max-width:100%; max-height:100%; object-fit:contain;">
                </div>
                <div class="thumbnail-bar" style="display:flex; gap:5px; margin-top:8px; overflow-x:auto; padding-bottom:4px;">
                    ${imgList.map((imgUrl, idx) => `<img class="thumb-img ${idx===0?'active':''}" src="${imgUrl}" onerror="this.src='https://via.placeholder.com/50';" onclick="swapCardImage('${key}', '${imgUrl}', this)" style="width:35px; height:35px; object-fit:cover; border-radius:4px; border:1px solid #cbd5e1; cursor:pointer;">`).join('')}
                </div>
                <div style="margin-top:10px; cursor:pointer;" onclick="openProductView('${key}')">
...
function handleVariantChange(key) {
    ...
    if (!prod.variants || !prod.variants[selectedIdx]) return;
...
function processSmartSearch() {
    ...
    if (query === "") {
        resultsGrid.innerHTML = `<p style="text-align: center; color: #94a3b8;">Type something above to search products...</p>`;
        return;
    }
    ...
    Object.keys(localGlobalProducts).forEach(key => {
        const product = localGlobalProducts[key];
        if (product && product.name && product.name.toLowerCase().includes(query)) {
            filteredData[key] = product;
            found = true;
        }
    });

    if (!found) {
        resultsGrid.innerHTML = `<p style="text-align: center; color: #94a3b8; grid-column: 1/-1;">No products found for "${searchInput.value}"</p>`;
        return;
    }

    // Temporarily hijack innerHTML rendering to search results grid
    const originalPDisplay = document.getElementById('productsDisplay');
    const originalODisplay = document.getElementById('offersDisplay');

    // Create virtual elements to reuse existing render function safely
    const virtualDiv = document.createElement('div');
    
    // Backup and swap IDs so renderProductGrids renders into results grid
    if(originalPDisplay) originalPDisplay.id = 'productsDisplay_backup';
    if(originalODisplay) originalODisplay.id = 'offersDisplay_backup';
    
    resultsGrid.id = 'productsDisplay'; // Route regular products to search grid
    virtualDiv.id = 'offersDisplay';   // Route offer products to virtual container

    resultsGrid.innerHTML = '';
    
    // Render the filtered data
    renderProductGrids(filteredData);

    // Append offers if any matched search into the same search results grid
    if(virtualDiv.innerHTML !== '') {
        resultsGrid.innerHTML += virtualDiv.innerHTML;
    }

    // Restore original element IDs
    resultsGrid.id = 'searchResultsGrid';
    if(originalPDisplay) originalPDisplay.id = 'productsDisplay';
    if(originalODisplay) originalODisplay.id = 'offersDisplay';
}
...
function clearSearchField() {
    ...
    if(resultsGrid) resultsGrid.innerHTML = `<p style="text-align: center; color: #94a3b8;">Type something above to search products...</p>`;
}
...
function addToCartEngine(key, launchDirectCheckout) {
    ...
    if (prod.variants && prod.variants.length > 0 && prod.variants[0].image) {
        finalImg = prod.variants[0].image;
    } else if (prod.images && prod.images.length > 0) {
        finalImg = prod.images[0];
    }

    const selectEl = document.getElementById(`sel-${key}`);
    if (selectEl) {
        const selectedIdx = parseInt(selectEl.value);
        if (prod.variants && prod.variants[selectedIdx]) {
            const chosenVariant = prod.variants[selectedIdx];
    ...
    const cartItem = { productId: key, name: finalName, price: finalPrice, qty: qty, img: finalImg };
    cart.push(cartItem);
    updateCartUIState();
    ...
    cart.forEach((item, index) => {
        ...
        container.innerHTML += `
            <div class="cart-item-row" style="display:flex; align-items:center; gap:10px; border-bottom:1px solid #e2e8f0; padding:10px 0;">
                <img src="${item.img}" onerror="this.src='https://via.placeholder.com/50';" style="width:45px; height:45px; object-fit:cover; border-radius:6px;">
                <div style="flex:1;">
...
function checkoutCart() {
    ...
    let orderMsg = `🧾 *OFFICIAL ORDER RECEIVED - Smart Shopper* 🧾\n\n`;
    orderMsg += `🆔 *Order ID:* ${generatedOrderId}\n`;
    orderMsg += `📅 *Date:* ${currentDate}\n\n`;
    orderMsg += `👤 *CUSTOMER DETAILS:*\n`;
    orderMsg += `📝 Name: ${name}\n`;
    orderMsg += `📞 Phone: ${phone}\n`;
    orderMsg += `📍 Delivery Address: ${address}\n\n`;
    orderMsg += `📦 *PURCHASED ITEMS:*\n`;

    cart.forEach(item => {
        const itemSum = item.price * item.qty;
        finalTotal += itemSum;
        orderMsg += `• ${item.name} (x${item.qty}) -> LKR ${itemSum.toLocaleString()}\n`;
        itemsStructure.push({ productId: item.productId, name: item.name, qty: item.qty, price: item.price, image: item.img });
    });
    orderMsg += `\n💰 *Total Price Amount:* LKR ${finalTotal.toLocaleString()}\n\n`;
    orderMsg += `🚀 *Kindly process this request as soon as possible!*`;

    const orderPayload = { orderId: generatedOrderId, customerName: name, customerPhone: phone, customerAddress: address, date: currentDate, items: itemsStructure, totalCost: finalTotal };

    fetch(FIREBASE_BASE_URL + "orders.json", { method: "POST", body: JSON.stringify(orderPayload) }).then(() => {
        ...
        setTimeout(() => {
            const adminWhatsAppLine = "94758487089"; 
            window.open(`https://wa.me/${adminWhatsAppLine}?text=${encodeURIComponent(orderMsg)}`, '_blank');
            cart = [];
            updateCartUIState();
        }, 1800);
    }).catch(err => { alert("Database transaction failed. Please retry submission."); });
}
...
// --- FIXED THEME SYSTEM WITH MOON AND SUN TOGGLE LOGIC ---
function toggleTheme() { 
    ...
    if(body.classList.contains('dark-theme')) {
        if(btn) btn.innerHTML = `<i class="fa-solid fa-sun" style="color: #f59e0b;"></i>`;
    } else {
...
function openProductView(key) { console.log("Product detail target view: " + key); }
