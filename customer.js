...
function loadProducts() {
    ...
    // Sync Live WhatsApp Channel Config Nodes
    ...
    // Sync Hero Custom Banner Data Layers dynamically from Admin Panel Settings Node
    fetch(FIREBASE_BASE_URL + "settings/heroBanner.json")
    .then(res => res.json())
    .then(banner => {
        if(banner && banner.title) {
            const bannerHeaderElement = document.querySelector('.hero-banner h2');
            const bannerTextElement = document.querySelector('.hero-banner p');
            if(bannerHeaderElement) bannerHeaderElement.innerText = banner.title;
            if(bannerTextElement) bannerTextElement.innerText = banner.subtitle || '';
        }
    }).catch(err => console.log("Banner dynamic load bypassed."));
...
function processSmartSearch() {
    ...
    resultsGrid.id = 'productsDisplay'; 
    virtualDiv.id = 'offersDisplay';   
...
function checkoutCart() {
    ...
    // Order init payload defaults orderStatus flag state to "Pending"
    const orderPayload = { 
