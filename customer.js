...
function loadProducts() {
    ...
    .then(res => res.json())
    .then(link => {
        if (link) document.getElementById('uiWhatsappLink').href = link;
    });
    ...
    .then(data => {
        if (!data) {
            productsDisplay.innerHTML = `<p class="no-products">Inventory is empty!</p>`;
        ...
        Object.keys(data).forEach(key => {
            ...
            if (product && product.name) {
                ...
                if (product.variants && Array.isArray(product.variants) && product.variants.length > 0) {
                    ...
                    selectorHTML += `<div style="margin: 12px 0; display:flex; gap:6px; justify-content:center; flex-wrap:wrap;">`;
                    product.variants.forEach((v, idx) => {
                        const activeBorder = idx === 0 ? 'border: 2px solid var(--primary-teal);' : 'border: 1px solid #cbd5e1;';
                        selectorHTML += `
                            <img src="${v.image}" class="var-thumb-${key}" onclick="changeProductVariantView('${key}', '${v.image}', '${v.name.replace(/'/g, "\\'")}', ${v.price}, ${v.offerPrice || 'null'}, this)" 
                            style="width:38px; height:38px; object-fit:cover; border-radius:6px; cursor:pointer; ${activeBorder}" title="${v.name}">
                        `;
                    });
                    selectorHTML += `</div>`;
                } else {
                    // Fallback configuration if standard pricing structure found
                    let fallPrice = product.price || 0;
                    initialPriceHTML = `<p class="prod-price" id="price_box_${key}">Rs. ${fallPrice}</p>`;
                    initialImage = product.image || 'https://via.placeholder.com/150';
                }
                ...
                const actionButton = isSoldOut 
                    ...
                    : `<button class="buy-now-btn" id="btn_${key}" onclick="addActiveVariantToCart('${key}', '${product.name.replace(/'/g, "\\'")}')"><i class="fa-solid fa-cart-plus"></i> Add to Luxury Cart</button>`;
                ...
                const firstVar = product.variants ? product.variants[0] : {name: 'Base', price: product.price || 0, offerPrice: null};
                const finalInitPrice = firstVar.offerPrice || firstVar.price;

                const productCardTemplate = `
                    <div class="product-card" ${cardStyle} id="card_${key}" 
                         data-selected-name="${firstVar.name}" 
                         data-selected-price="${finalInitPrice}" 
                        ...
                        <h4>${product.name}</h4>
                        <p class="prod-id" id="variant_title_${key}" style="font-weight:700; color:#64748b; font-size:12px; margin:2px 0;">Option: ${firstVar.name}</p>
                        ${selectorHTML}
                        ${initialPriceHTML}
        ...
        if(!hasOffers) {
            offersDisplay.innerHTML = `<p class="no-products" style="grid-column: 1/-1;">No special flash sales active.</p>`;
        }
    })
    .catch(err => console.error("Database mismatch error:", err));
}
...
function changeProductVariantView(productKey, img, varName, price, offerPrice, thumbElement) {
    ...
    document.getElementById('variant_title_' + productKey).innerText = "Option: " + varName;
...
function addActiveVariantToCart(productKey, mainName) {
    ...
    if(existingItem) {
        ...
        cart.push({ cartKey: trackingCartKey, key: productKey, name: mainName, variantName: varName, price: finalPrice, image: img, quantity: 1 });
    }
    updateCartUI();
...
function handleOrderSubmit(e) {
    ...
    const orderPayload = {
        orderId: orderId, name: custName, address: custAddress, phone: custPhone, items: cart, total: totalAmount, date: new Date().toLocaleDateString(), status: 'Pending'
    };
    ...
    .then(() => {
        ...
        cart.forEach(i => { emailBody += `- ${i.name} [${i.variantName}] (Qty: ${i.quantity})\n`; });
        
        const mailtoUrl = `mailto:smartshopperindustry@gmail.com?subject=NEW ORDER ${orderId}&body=${encodeURIComponent(emailBody)}`;
        ...
        toggleCartModal(); toggleReceiptModal();
    });
}
...
function moveTarget() {
    const target = document.getElementById('targetBulb'); if(!target) return;
    target.style.transform = `translate(${Math.floor(Math.random() * 200) - 100}px, ${Math.floor(Math.random() * 200) - 100}px)`;
}
function hitTarget() {
    gameScore++; document.getElementById('gameScore').innerText = gameScore;
    if(gameScore === 5) { gameScore = 0; toggleGameModal(); } moveTarget();
}
